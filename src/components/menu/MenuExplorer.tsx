...
import {animatedCategoryFeatures} from '@/data/menu-animation-data';
import {DietIcon} from '@/components/ui/Motion';
import {MenuItemModal} from './MenuItemModal';
import {CategoryFoodFeature} from './food-animation/CategoryFoodFeature';
import {MenuFoodStory} from './food-animation/MenuFoodStory';
import {useFoodMotionPreference} from './food-animation/useAnimationBudget';
import {AnimatedMenuRow} from './AnimatedMenuRow';
import {ScrollRail} from './ScrollRail';
const groups=['All','Pizza','Chicken','Burgers','Rice','Snacks','Salads','Drinks','Desserts'];
export function MenuExplorer(){
 const [motionPaused,toggleMotion]=useFoodMotionPreference();
 ...
 const markPlayed=useCallback((id:string)=>setPlayed(previous=>previous.has(id)?previous:new Set([...previous,id])),[]);
 ...
 return <><div ref={root} className="menu-experience live-menu-experience">
  ...
  <div className="menu-layout"><aside className="category-nav"><span className="eyebrow">WHAT ARE YOU CRAVING?</span><ScrollRail className="category-rail" label="Menu categories"><nav aria-label="Menu categories">{categories.map(c=><a className={active===c.id?'active':''} aria-current={active===c.id?'location':undefined} href={`#${c.id}`} key={c.id} onClick={()=>setActive(c.id)}>{c.name}<span>{c.items.length.toString().padStart(2,'0')}</span></a>)}</nav></ScrollRail><a className="download-menu" href="/menu/mozza-italia-menu.pdf" target="_blank" rel="noreferrer"><Download size={16}/> Original menu PDF</a>{group==='All'&&!query&&diet==='all'&&<a href="#food-story" className="live-story-jump"><Sparkles size={15}/> From craving to plate <ArrowUpRight size={15}/></a>}</aside>
   ...
   {categories.map(c=>{const featureConfig=animatedCategoryFeatures[c.id];const feature=featureConfig&&!query?featureConfig.itemIds.map(id=>c.items.find(i=>i.id===id)).find(Boolean):undefined;
    return <section className="menu-section" id={c.id} key={c.id} data-menu-section><div className="menu-section-title"><h2>{c.name}</h2><span>{c.items.length.toString().padStart(2,'0')}</span></div>{c.note&&<p className="category-note">{c.note}</p>}
     {feature&&<CategoryFoodFeature item={feature} sizeIndex={featureConfig.sizeIndex} onSelect={()=>setSelected({item:feature,category:c.name,sizeIndex:featureConfig.sizeIndex})} alreadyPlayed={played.has(`${c.id}-${feature.id}`)} onPlayed={()=>markPlayed(`${c.id}-${feature.id}`)}/>}
     <div className="menu-rows">{c.items.map((item,index)=><AnimatedMenuRow key={item.id} item={item} index={index} onSelect={()=>setSelected({item,category:c.name})}/>)}</div>
    </section>;})}
 ...
 </div>{group==='All'&&!query&&diet==='all'&&<MenuFoodStory onSelect={showItem}/>}</>;
}
